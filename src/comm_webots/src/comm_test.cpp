...
#include <comm_webots/set_int.h>

#include <comm_webots/set_float.h>
...
int main(int argc, char **argv) {
  ...
  int stepMax = 1;
  int nStep = 0;
  ros::init(argc, argv, "SAKURA", ros::init_options::AnonymousName);
  ros::NodeHandle n;
 ...
 // Wait for the `ros` controller.
  ...
  comm_webots::set_int enableDistSensorSrv;


  ros::ServiceClient rightWheelPositionClient = n.serviceClient<comm_webots::set_float>(std::string("SAKURA/")+ std::string("/FootR/set_position"));
  ros::ServiceClient leftWheelPositionClient = n.serviceClient<comm_webots::set_float>(std::string("SAKURA/")+ std::string("/LegLowerL/set_position"));
   comm_webots::set_float wheelSrv;
     wheelSrv.request.value = 10 ;
